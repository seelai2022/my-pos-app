    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">ຈັດການສິນຄ້າ</h1>
            <p className="text-sm text-gray-400 mt-0.5">{products.length} ລາຍການ</p>
          </div>
          <div className="flex gap-2 flex-wrap justify-end">
            <button onClick={() => setShowUnitManager(true)}
              className="px-3 py-2.5 border border-gray-200 bg-white text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-50">
              ⚙️ Unit
            </button>

            {/* Export Excel */}
            <button onClick={handleExport}
              className="flex items-center gap-1.5 px-3 py-2.5 border border-green-200 bg-green-50 text-green-700 text-sm font-medium rounded-xl hover:bg-green-100 transition-colors">
            ...
            {/* Import Excel */}
            <label className="flex items-center gap-1.5 px-3 py-2.5 border border-blue-200 bg-blue-50 text-blue-700 text-sm font-medium rounded-xl hover:bg-blue-100 transition-colors cursor-pointer">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left px-5 py-3 text-xs font-medium text-gray-400">ສິນຄ້າ</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-gray-400">ລາຄາ</th>
                    <th className="text-center px-5 py-3 text-xs font-medium text-gray-400">Units</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-gray-400">ສາງ</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {products.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center shrink-0">
                            {p.image_url ? <img src={p.image_url} alt={p.name} className="w-full h-full object-cover"/> : <span className="text-2xl">{p.emoji}</span>}
                          </div>
                          <div>
                      ...
                      <td className="px-5 py-3 text-right text-sm text-gray-700">{p.price.toLocaleString()} ₭</td>
